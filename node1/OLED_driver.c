...
void OLED_print_number(uint32_t c)
{
    if (c > 9000)
    {
        OLED_print("It's over 9000! :D");
        return;
    }
    int thousands = c / 1000;
    int hundreds = (c - 1000 * thousands) / 100;
    int tens = (c - 100 * hundreds) / 10;
    int ones = (c - 10 * tens) / 1;
    if (thousands)
    {
        for (int i = 0; i < fontsize; i++)
        {
            OLED_write_data(pgm_read_byte(&font4[thousands + 16][i]));
        }
    }
    if (thousands || hundreds)
    {
        for (int i = 0; i < fontsize; i++)
        {
            OLED_write_data(pgm_read_byte(&font4[hundreds + 16][i]));
        }
    }
    if (thousands || hundreds || tens)
    {
        for (int i = 0; i < fontsize; i++)
        {
            OLED_write_data(pgm_read_byte(&font4[tens + 16][i]));
        }
    }
    for (int i = 0; i < fontsize; i++)
    {
        OLED_write_data(pgm_read_byte(&font4[ones + 16][i]));
    }
}
